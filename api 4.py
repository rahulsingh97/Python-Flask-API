from flask import *
import json


app = Flask(__name__)


@app.route('/')
def home_page():

    data = {'Page': 'Home', 'Message': 'hi'}

    jsondump = json.dumps(data)


    return jsondump


@app.route('/name/', methods=['GET'])
def name_page():


    userquery = str(request.args.get('thename')) #/name/?thename=

    data = {'Page': 'Name', 'Your name is': f'{userquery}'}

    jsondump = json.dumps(data)


    return jsondump

@app.route('/nameage/', methods=['GET'])
def aa_page():


    userquerya = str(request.args.get('thename')) #/nameage/?thename=&theage=
    userqueryb = str(request.args.get('theage'))

    data = {'Name': f'{userquerya}', 'Age': f'{userqueryb}'}

    jsondump = json.dumps(data)


    return jsondump

@app.route('/nameandage/', methods=['GET'])
def namepage_page():

    userresponse = request.args.to_dict()  #/nameandage/?name=&age=

    name = userresponse['name']
    age = userresponse['age']


    data = {'Your name is': f'{name}', 'Your age is': f'{age}'}

    jsondump = json.dumps(data)


    return jsondump


if __name__ == '__main__':
    app.run(port=5000, debug=True)